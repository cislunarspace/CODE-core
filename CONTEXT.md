...
### 圆锥曲线拼接（Patched conic）
把地月转移拆成若干段、每段以单一中心天体的二体圆锥曲线近似、在交接点拼接的
设计方法。PCN 转移类型即此近似：地球段为地心二体双曲出发，月球段为月心二体
双曲到达，交接点月心态由地球段传播减月球惯性态得到。
_避免_：分段圆锥、拼接圆锥

### 双曲渐近线（Hyperbolic asymptote）
双曲轨道在无穷远的出射/入射速度方向，按赤经 RHA、赤纬 DHA 与 C3 = v∞²
（单位 km²/s²）参数化。出发渐近线为地球段出射速度方向；到达渐近线为月球段
入射速度方向。RHA ∈ [0°, 360°)、DHA ∈ [−90°, 90°]。
_避免_：v-infinity 矢量、逃逸方向

### B 平面（B-plane）
以双曲入射渐近线方向 Ŝ 为法向、过中心天体的平面（Vallado 定义）。基矢
T̂ = normalize(ẑ×Ŝ)、R̂ = Ŝ×T̂；B 矢量为从中心天体指向入射渐近线与 B 平面交点
的向量，模为瞄准距离 b，分量记作 B·R、B·T（月心 GCRS 惯性系，单位 km）。
近月点高度 + B·T 唯一确定月心双曲交会几何，是 PCN 到达模式的目标参数。
_避免_：B 向量平面、冲击参数平面

### TLI / LOI
Trans-Lunar Injection（地月转移轨道注入）与 Lunar Orbit Insertion（环月轨道
注入）脉冲。TLI 为地球停泊轨道上的出发脉冲（PCN 按共面圆停泊轨道理想化），
LOI 为近月点圆化脉冲。
_避免_：地月转移注入/环月注入（正文可用中文，术语键保留英文缩写）
...
### 月心分支（Moon-centered branch）
判据级联中先于共线平动点分支的分支；绕月净卷绕至少一周、全程月距在月球影响球内、且月心顺逆可判的周期轨道（平面或空间）取四个月心标签（`distant_retrograde` / `distant_prograde` / `low_prograde_eastern|western`），周期通约时追加辅助共振标签；空间与平面输入共用同一标签选择。
_避免_：平面月心分支（自 #707 起不限平面）

### 月心不可判（moon_centered_indeterminate）
`unclassified_reason` 取值之一；空间、全程月距在影响球内、不卷绕任何共线平动点、但月心判据不可判（近月点角动量 z 分量退化，如极轨/近极地）的输入的显式未分类原因，此类输入绝不贴平动点族标签。
_避免_：极轨月心标签（词汇表无此标签；语义扩展另开决策）
