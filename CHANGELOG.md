...
- **RTN 常值加速度力模型 `UniformAcceleration`**：新增与航天器质量无关的 RTN 三轴常值加速度（匀加速度）力模型，对应 CE-5 精密定轨策略（孔静等 2022）中的姿轨控匀加速度项。`UniformAcceleration(acceleration_rtn, direction_frame="RTN")` 在每个 RHS 求值处以当前状态的 RTN（RSW）基把 (aR, aT, aN) 旋转到传播坐标系，轴约定与 `FiniteBurn(direction_frame="LVLH")` 同为 R = r/|r|、N = (r×v)/|r×v|、T = N×R（沿迹轴垂直于径向）。支持 `ForceModel.from_config`/`to_config` 配置往返（type 名 `UniformAcceleration`，参数 `acceleration_rtn`/`direction_frame`）与 `with_stm=True` 变分传播；三分量全零等价于无该力，退化状态（|r|≈0、|v|≈0，或 r∥v 使 |r×v|≈0）在传播中显式报错、不静默丢弃分量（与 `direction_frame="LVLH"` 推力帧不同：后者共线时丢弃 N 分量）。(#667)
- **EPPR（星历脉动旋转系）转换与动力学**：新增 Earth–Moon EPPR——x̂ 沿瞬时地月连线、ẑ 沿瞬时轨道角动量、ŷ 右手补全，长度尺度取瞬时地月距离 d(t)。`spacetime_transform` 新增 `j2000_to_eppr`／`eppr_to_j2000`：输入输出与 synodic 同约定（地心 J2000 的 km／km·s⁻¹ ↔ **质心**无量纲 EPPR），`times` 为相对 `et0_jd` 的无量纲时间（t* = 375190.2588926273 s、l* = 384400 km），地球固定于 EPPR 无量纲 (−μ,0,0)、月球固定于 (1−μ,0,0)。算法层新增 `EPPRSystem`/`EPPR_Dynamics`：双主星引力 + 旋转-脉动系全部惯性项（科氏/离心/Euler/质心平动/脉动），帧量由真实星历（`SPICEManager.get_body_state`）解析推导（ḋ、d̈、ω、ω̇），速度含旋转角速度项与 ḋ/d 脉动项；圆形星历下精确退化为 CR3BP。时变系统无 Jacobi 积分、不提供 STM 与 Rust 快速路径，传播走 scipy。(#650)
...
- **NRLMSISE-00 大气密度模型**：新增 `NRLMSISE00Atmosphere`（`e2m2e.algorithm.forces.atmosphere`），供地球停泊弧段的阻力建模使用。大气密度接口从"纯高度"扩为"高度 + 历元（年积日/UT）+ WGS84 大地经纬度 + 空间天气"；`ExponentialAtmosphere` 增加同签名的仅关键字参数（其自身忽略），仍是缺省模型。`DragModel` 接受两种大气模型，配置里以 `{"type": "NRLMSISE00Atmosphere", "params": {"f107_daily": …, "f107_avg": …, "ap": …}}` 选择：`ap` 传标量（或 7 元全等序列）即静态空间天气，传 7 元非全等序列则启用 3 小时分辨率的暴时先验；`f107_avg` 缺省取 `f107_daily`。密度口径为 drag 有效总质量密度（含 500 km 以上不可忽略的异常氧）。高度域与指数模型一致（0–1000 km，负高度钳到 0，1000 km 及以上返回 0）。历元换算优先走星历预采样缓存（`EphemCache` 新增 ET→UTC 预采样表），因此 NRLMSISE-00 阻力可用于打靶/分段积分的**并行**区而保持"零 cspice"（ADR 0016）；缓存未启用时回退 `et2utc`，故仍需已装载 SPICE 内核（含 leap second 内核），内核池为空时显式报错。数值与官方参考实现（NRL 公有领域 C 版，f64）在 nyx 公开的 72 工况验证集上逐位一致。(#637)
...
### Fixed
- **空间月心轨道不再被误标为平动点族**：`classify_orbit` 的月心分支原要求轨道全程 `max|z| ≤ 1e-7`（严格平面），空间（z≠0）、绕月净卷绕且全程月心距在月球影响球内的轨道因此无法命中月心分支而落入共线回退，被误贴 `halo_l*`/`axial_l*`/`vertical_l*`（如空间顺行月心大圆误标 `halo_l2_southern`）。现月心分支接受空间输入并与平面输入共用同一标签选择：绕月净卷绕 ≥1.5π、全程月心距 ≤ μ^(2/5)（Chebotarev 影响球）、且月心顺逆可判（近月点角动量 z 分量非退化）的轨道取 `distant_retrograde` / `distant_prograde` / `low_prograde_eastern|western`，周期通约时照旧追加 `resonant_*` 辅助标签。空间、全程月心距在影响球内、不卷绕任何共线平动点、但月心判据不可判（近月点角动量 z 分量退化，如极轨/近极地）的输入改报显式 `unclassified_reason="moon_centered_indeterminate"`（此前误贴 `halo_l1_northern` 一类平动点标签），此类输入绝不贴平动点族标签；卷绕共线平动点的空间轨道（含完全落在影响球内的小型 halo）仍走共线分支。平面输入与随包 baseline 11 个周期族 392 个成员的标签逐位不变。(#707)
...
### Added
- **共振轨道（RO）设计与族生成落地**：`design_orbit(orbit_type="RO")` 与 `orbit_family_generation(orbit_type="RO")` 自此可用——按恒星共振比 `resonance_p:resonance_q`（航天器惯性圈数:月球圈数，Vaquero & Howell 2014 式（7）；支持 2:1/3:1/3:2/4:1/4:3 五档顺行内共振，缺省 3:1）生成绕地质心的顺行平面周期轨道。闭合条件为 q 个恒星月内绕地 p 圈：会合系周期 `T = 2πq`、净卷绕 `w = p−q`（如 3:1 图案周期约 27.3 天、惯性周期约 9.1 天；4:3 约 82 天），五档精确成员均锚定文献偏心族，并以闭合误差、Kepler 半长轴量级、偏心形态与卷绕数四重守卫拒绝伪支。可选 `amplitude`（距地心距离 min/max 均值，约 145,000–340,000 km 包络）沿族行走命中目标，族生成按振幅窗口收集成员；无法命中的窗口诚实报错。`valid_ranges` 同步导出 RO 的振幅包络与共振比值域。DRO→RO 转移测试改用 `design_ro` 生成的 3:1 RO，不再依赖外部数据文件。(#627)
- **Sphinx 在线文档站上线**：接口参考与教程文档发布于 https://cislunarspace.github.io/CODE-core/ ，随 master 合并自动部署；文档构建不编译 Rust 内核、维持零告警基线。(#651)
...
- **DRO 种子常量统一为收敛周期轨道真值**：测试、Rust 与生产侧 DRO 初猜统一为 vy0=0.536819842572739、周期 T=3.472535773770595（x0 保持 0.79188556619742），校正 `design_dro` 文档示例的振幅口径（距月心距离 min/max 均值），并新增一周期闭合回归测试锁死真值。(#656, #657, #658)
- **转移 NLP 松弛速度约束符号反置**：`Transfer.optimize` 的 SLSQP 后端在默认松弛速度约束下把不等式方向写反（实际强制速度夹角远离平行），求解器在插入速度与目标轨道速度近乎相反的物理不可行解上照报 CONVERGED；现约束方向与“夹角不超过容差”一致，不可行时如实报 INFEASIBLE/CONSTRAINT_VIOLATION。
- **Halo 伪弧长延拓的切向定向与镜像支回正**：`halo_pseudo_arclength_continuation` 逐步切向量（雅可比零空间基，数值符号任意）未与上一步对齐，预测步会沿族倒走（振幅 ±步长 回摆、跨 z=0 镜像跳支，北族延拓不再单调）；现逐步同向化，并把落在 z 镜像支的解按 CR3BP 镜像对称折回目标半球，正/负向延拓的振幅单调性恢复。

### Internal
- Linux 下 `make test-rust` 固定项目 venv Python 并把其 LIBDIR 注入动态库加载路径；取不到 LIBDIR 时明确失败退出。(#660)
